...
data <- data[, order(sapply(data, is.factor)), order(sapply(data, is.numeric))]
```
...
> The comparison between mean values with and without outliers shows how strongly the outliers influence the average values in most variables.
<br>
NaN in revol_util could mean that all values of this variable were considered outliers or could be missing values. As we have already checked the variables for NA values, we can rule out the possibility that these are missing values. The distribution of this variable is very different from the other variables. The calculation of the average of the outliers could cause difficulties due to certain characteristics of the distribution, as no clear upper and lower limit for outliers can be defined.
...
```{r}
# Liste für robuste Modelle
robust_models <- list()

# Entferne Zeilen mit fehlenden Werten in der Zielvariablen
data <- na.omit(data)
data$Status <- as.numeric(as.character(data$Status))

# Iteriere über jede numerische Variable
for (variable in numeric_variables) {
  # Erstelle robustes Modell
  robust_model <- rlm(Status ~ get(variable), data = data)
  
  # Speichere das Modell in der Liste
  robust_models[[variable]] <- robust_model
  
  # Zeige die Zusammenfassung
  cat("Robustes Modell für", variable, ":\n")
